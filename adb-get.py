...
def observations_sandbox_api_key():
    """Value of the Observations SANDBOX API key environment variable if it exists, otherwise None."""
...
def ping_apis (species_api_key, observations_api_key, observations_sandbox_api_key, verbose=False):
    ...
    print('Artdatabanken Species API. URL: https://api-portal.artdatabanken.se/Products/species-information')
    ...
    print('Artdatabanken Observations SANDBOX API. URL: https://api-portal.artdatabanken.se/Products/sandbox-species-read')
    print('GET %s' % ping_url)
    print('HTTP Status code: %s' % (r.status_code))
...
def main():
    ...
    parser.add_argument('--observations-sandbox-api-key', default=observations_sandbox_api_key(),
                        help="Observations SANDBOX API authentication (subscription) key [value of environment variable %s]" %
    ...
    if args.ping_the_apis:
        ping_apis(args.species_api_key, args.observations_api_key, args.observations_sandbox_api_key, args.verbose)
        sys.exit(0)
    if args.get_observations:
        ...
        if not args.use_production_api:
            ...
            print("Info: Using observations API. NOTE: This currently just does a predefined search and ignores the CLI arguments!")
