...
class AdbObsAPI():
    ...
    def area_types (self):
        """Valid area types as documented here:
           ...
           in the form of a dictionary indexed by area type id, where the entries are Swedish (sv-SE)
           and English (en-GB) names of the area types. Use the area id type when calling the API.
           By calling areas with type codes 1...1000 you get slightly different area types than
           documented above. The returned dictionary reflects all those area types returned by
           that API call."""
    ...
    def observations (self, search_filter, index=0, count=10, sort_by=None, sort_order="Desc", lang="sv-SE", sensitive=False, verbose=False):
        """Observations matching 'search_filter'. Yeah, butt ugly."""
        assert index >= 0
        assert count <= 1000
        url = "%s%s%s?skip=%d&take=%d&" % (API_ROOT_URL, API_ROOT_PATH, "Observations/Search", index, count)
        if sort_by:
        ...
        r = requests.post(url, data=search_filter, headers={**auth_headers(self.api_key), **{"Content-Type": "application/json"}})
        if verbose:
            print_http_response(url, r)
        return r

    def observation (self, observation_id, output_field_set="All properties", lang="sv-SE", sensitive=False, verbose=False):
